...
export default function QuestionnaireStepPage() {
  ...
  const goToStep = (next: number, direction: 'forward' | 'back') => {
    ...
    setTimeout(() => {
      router.push(`/questionnaire/${next}`);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-black md:min-h-screen">
      <Header />
      
      {/* Header */}
      <header className="px-6 py-6 md:pt-12 flex-shrink-0">
      ...
      {step > 2 && state.sport && state.event && isShortDistanceEvent(state.sport, state.event) && (
        <div className="sticky top-0 z-10 px-6 py-3 bg-white flex-shrink-0">
          <div className="max-w-xl mx-auto">
      ...
      {/* Content - Mobile: fixed question, scrollable answers only */}
      <main className="flex-1 flex flex-col md:items-center md:justify-center px-6 py-6 md:py-12 pb-20 md:pb-12 overflow-hidden">
        <div className="w-full max-w-xl mx-auto flex flex-col h-full md:h-auto">
          {/* Question text - fixed on mobile only */}
          <div className="flex-shrink-0 mb-6 text-center md:hidden">
            {step === 1 && <QuestionHeader title="Was ist deine Hauptsportart?" description="Wähle die Sportart, für die du aktuell trainierst." />}
            {step === 2 && <QuestionHeader title="Welche Wettkampfdistanz?" description="Wähle den Wettkampf, auf den du dich vorbereitest." />}
            {step === 3 && <QuestionHeader title="Was ist deine aktuelle Zeit für diese Distanz?" description={`Für ${state.event ? eventDisplayNames[state.event as EventType] : 'dein Event'}`} />}
            {step === 4 && <QuestionHeader title="Aktuelle Kohlenhydratmenge im Wettkampf?" description="Schätze, wie viele Gramm Kohlenhydrate du pro Stunde im Schnitt zuführst." />}
            {step === 5 && <QuestionHeader title="Wie häufig hast du Magen-Darm-Beschwerden?" description="Im Wettkampf oder bei intensiven Trainingssessions." />}
            {step === 6 && <QuestionHeader title="Biologisches Geschlecht" description="Wird genutzt, um die Stoffwechsel-Berechnung fein anzupassen." />}
          </div>
          
          {/* Answer options - scrollable on mobile only */}
          <div className="flex-1 overflow-y-auto md:overflow-visible md:flex-none">
            <div
              className={`w-full question-container ${
            ...
            >
              ...
              {step === 6 && <Question6Gender onSelect={() => router.push('/results')} />}
            </div>
          </div>
        </div>
      </main>

      {/* Navigation Footer - Sticky on mobile, always at bottom */}
      <footer className="fixed bottom-0 left-0 right-0 md:relative md:bottom-auto md:left-auto md:right-auto bg-white border-t border-black/5 md:border-t-0 px-6 py-4 md:py-6 md:pb-16 z-20 flex-shrink-0 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] md:shadow-none">
...
function QuestionHeader({ title, description }: { title: string; description: string }) {
  return (
    <div>
